...
class CompetitorsSearch:
    ...
    def get_images_names(self, ) -> Tuple[List[Image.Image], List[object]]:
        images, names, problems = list(), list(), list()
        for row in self.df.iterrows():
            row_num = row[0]
            row = row[1]
            img1 = self.get_sku_image(row.image_url_first)
            img2 = self.get_sku_image(row.image_url_second)
        ...
        self.images = images
        self.names = names
        self.im_problems = problems
        id_to_del = ~self.df.index.isin(problems)
        self.df = self.df[id_to_del].reset_index(drop=True)
        self.y = self.y[id_to_del].reset_index(drop=True)
        if self.is_debug:
            print(f'ids of images that did not open: {problems}')
